...
export const getWeatherApi = (cityName = "mumbai") => {
	return function (dispatch) {
		fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${REACT_APP_WEATHER_APP_ID}&units=metric`)
			.then((response) => {
...
export const getForcastApi = (cityName = "mumbai") => {
	return function (dispatch) {
		fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&appid=${REACT_APP_WEATHER_APP_ID}&units=metric`)
